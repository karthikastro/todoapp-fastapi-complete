...
SECRET_KEY = "mysecretkey20220817key@#$"
ALGORITHM  = "HS256"
...
async def create_user(createuser : CreateUser, db: Session= Depends(get_db)):
    ...
    hash_password = get_hash_password(createuser.password)
...
async def register_user(request :Request,email:str=Form(...),username:str= Form(...),first_name:str= Form(...),last_name : str=  Form(...),password:str = Form(...),password2:str =Form(...),db : Session=Depends(get_db)):
